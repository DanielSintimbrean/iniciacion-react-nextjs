...
- Usar `useState` para la información que queremos cambiar (estado) en el
  componente (como contadores, formularios, o estados de botones).
- Añadir interactividad que respondan a las acciones del usuario (clic,
  escribir, etc.).
- Actualizar el estado correctamente usando la función de actualización,
...
a renderizar el componente y el número nunca se actualiza en la UI.  
Debes usar `useState` para que los cambios sean reactivos.
...
return <button onClick={incrementar}>Has hecho clic {count} veces</button>;
```
